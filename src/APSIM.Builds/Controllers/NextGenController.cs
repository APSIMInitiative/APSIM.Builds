...
public class NextGenController : ControllerBase
{
    ...
    [Authorize]
    public async Task<IActionResult> UploadDocsAsync([FromForm] IFormFile document, uint pullRequestNumber)
    {
        ...
        string outputFile = Path.Combine(outputPath, document.FileName);

        using (Stream output = System.IO.File.Open(outputFile, System.IO.FileMode.Create))
            using (Stream input = document.OpenReadStream())
                await input.CopyToAsync(output);
